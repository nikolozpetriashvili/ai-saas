...
//mizezi imisa tu rato ar qreba mesijebi rodesac ai-s vekitxebit ragacas aris client-side router(react-router-dom), is yovel shecvill state-ze an raime msgavze aaxlebs mxolod komponents da ara mtlian page-s rac gamoricxavs statebis darefreshebis albatobasa da sxva danarcheni cvladebisa isev imave mnishvnelobebze dabrunebas(database-i tu gvaqvs rac ar unda varefreshot mteli page mainc kvelaferi isev ise gveqneba radganac vinaxav monacemebs mand).
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className='rounded-lg border w-full p-4 px-3 md:px-6 focus-within:shadow-sm grid grid-cols-12 gap-2'>
              <FormField name='prompt' render={({field})=> (<FormItem className='col-span-12 lg:col-span-10'>
                <FormControl className='m-0 p-0'>
                  <Input className='border-0 outline-none focus-visible:ring-0 focus-visible:ring-transparent' disabled={isLoading} placeholder='How do I calculate the radius of a circle?'
                  {...field} 
                  />
                </FormControl>
              </FormItem>)} />
